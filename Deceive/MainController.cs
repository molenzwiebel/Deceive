...
namespace Deceive
{
    ...
    internal class MainController : ApplicationContext
    {
        ...
        private void UpdateTray()
        {
            ...
            EnabledMenuItem = new ToolStripMenuItem("Enabled", null, async (a, b) =>
            {
                Enabled = !Enabled;
            ...
            var mucMenuItem = new ToolStripMenuItem("Enable lobby chat", null, (a, b) =>
            {
                ConnectToMuc = !ConnectToMuc;
                UpdateTray();
            }) { Checked = ConnectToMuc };

            ChatStatus = new ToolStripMenuItem("Online", null, async (a, b) =>
            {
            ...
            OfflineStatus = new ToolStripMenuItem("Offline", null, async (a, b) =>
            {
            ...
            MobileStatus = new ToolStripMenuItem("Mobile", null, async (a, b) =>
            {
            ...
            var restartWithDifferentGameItem = new ToolStripMenuItem("Restart and launch a different game", null,
                async (a, b) =>
                {
            ...
            var quitMenuItem = new ToolStripMenuItem("Quit", null, async (a, b) =>
            {
                var result = MessageBox.Show(
...
#if DEBUG
            var closeIn = new ToolStripMenuItem("Close incoming", null, (a, b) => { Incoming.Close(); });
            var closeOut = new ToolStripMenuItem("Close outgoing", null, (a, b) => { Outgoing.Close(); });
            var createFakePlayer = new ToolStripMenuItem("Resend fake player", null,
                async (a, b) => { await SendFakePlayerPresenceAsync(); });
            var sendTestMsg = new ToolStripMenuItem("Send message", null,
                async (a, b) => { await SendMessageFromFakePlayerAsync("Test"); });

            TrayIcon.ContextMenuStrip.Items.AddRange(new ToolStripItem[]
            {
                aboutMenuItem, EnabledMenuItem, typeMenuItem, mucMenuItem, closeIn, closeOut, createFakePlayer,
                sendTestMsg, restartWithDifferentGameItem, quitMenuItem
            });
#else
        ...
        private async Task IncomingLoopAsync()
        {
            try
            {
                ...
                do
                {
                    ...
                    {
                        if (content.IndexOf("offline", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            if (!Enabled)
                                await SendMessageFromFakePlayerAsync("Deceive is now enabled.");
                            OfflineStatus.PerformClick();
                        }
                        else if (content.IndexOf("mobile", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            if (!Enabled)
                                await SendMessageFromFakePlayerAsync("Deceive is now enabled.");
                            MobileStatus.PerformClick();
                        }
                        else if (content.IndexOf("online", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            if (!Enabled)
                                await SendMessageFromFakePlayerAsync("Deceive is now enabled.");
                            ChatStatus.PerformClick();
                        }
                        else if (content.IndexOf("enable", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            if (Enabled)
                        ...
                        else if (content.IndexOf("disable", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            if (!Enabled)
                        ...
                        else if (content.IndexOf("status", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            if (Status == "chat")
                        ...
                        else if (content.IndexOf("help", StringComparison.InvariantCultureIgnoreCase) >= 0)
                        {
                            await SendMessageFromFakePlayerAsync(
                                "You can send the following messages to quickly change Deceive settings: online/offline/mobile/enable/disable/status");
                        }
                ...
                do
                {
                    ...
                    if (!InsertedFakePlayer && content.Contains(roster))
                    {
                        ...
                        content = content.Insert(content.IndexOf(roster, StringComparison.Ordinal) + roster.Length,
                            ...
                            "</item>");
                        var outBytes = Encoding.UTF8.GetBytes(content);
                        await Incoming.WriteAsync(outBytes, 0, outBytes.Length);
            ...
            try
            {
                ...
                foreach (var presence in xml.Root.Elements())
                {
                    ...
                    if (ValorantVersion is null)
                    {
                        ...
                        if (valorantBase64 != null)
                        {
                            var valorantPresence = Encoding.UTF8.GetString(Convert.FromBase64String(valorantBase64));
                            var valorantJson = (JsonObject)SimpleJson.DeserializeObject(valorantPresence);
                            ValorantVersion = valorantJson["partyClientVersion"] as string;
                            Trace.WriteLine("Found VALORANT version: " + ValorantVersion);
                            // only resend
                            if (InsertedFakePlayer && ValorantVersion != null)
                                await SendFakePlayerPresenceAsync();
                        }
                    }
                ...
                await Outgoing.WriteAsync(outBytes, 0, outBytes.Length);
        ...
        private async Task SendFakePlayerPresenceAsync()
        {
            ...
            var valorantPresence = Convert.ToBase64String(
                Encoding.UTF8.GetBytes(
                    $"{{\"isValid\":true,\"partyId\":\"00000000-0000-0000-0000-000000000000\",\"partyClientVersion\":\"{ValorantVersion ?? "unknown"}\"}}")
            );
        ...
        private async Task SaveStatusAsync() => File.WriteAllText(StatusFile, Status);
